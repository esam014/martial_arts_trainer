...
function scorePunch(elbowMin, speed, balErr, rot) {
    const extension = Math.min(100, (170 - elbowMin) * 2);
    const velocity = Math.min(100, speed * 6000);
...
function onResults(res) {
    ...
    if (!canvasInitialized) resizeCanvas();
...
function voiceCoach(punch, score, lm, stance) {
    ...
    if (punch && punch.includes("HOOK") && score < 75)
        return "Rotate and keep your elbow up";
